          ...
          return (
            <div key={a.id} className="act-row" style={{ display: "flex", alignItems: "center", height: ROW_H, padding: "0 12px", borderBottom: "1px solid #21262d", background: hoveredId === a.id ? "#161b22" : "#0d1117", gap: 8 }}
              ...
              <div style={{ display: "flex", gap: 4, opacity: hoveredId === a.id ? 1 : 0, transition: "opacity .15s" }}>
                <button onClick={() => openEdit(a)} style={{ background: "none", border: "none", color: "#58a6ff", cursor: "pointer", fontSize: 13, padding: 2 }}>Edit</button>
                <button onClick={() => setDeleteConfirm(a.id)} style={{ background: "none", border: "none", color: "#f85149", cursor: "pointer", fontSize: 13, padding: 2 }}>Del</button>
              </div>
            </div>
          );
        })}
